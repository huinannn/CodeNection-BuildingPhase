    ...
    <div class="page">
        <div class="head">
            ...
            <div class="subtitle">
                ...
                <h1 class="active">Comments</h1>
                <div class="vertical"></div>
                <h1 onclick="window.location.href='reviews_reply_sad.php'">Replies</h1>
                <div class="spacer"></div>
                <div class="dark_light">
                    <input type="checkbox" checked onchange="window.location.href='reviews_comment_happy.php'">
                </div>
            </div>
        </div>
        <div class="sort">
        ...
        <div class="table">
            ...
            <div class="table_data" data-status="all">
                <?php
                    $all_sql = "SELECT cm.*, s.student_name
                                FROM comment cm
                                JOIN confession c 
                                ...
                                ORDER BY cm.comment_date_time DESC;";
                ...
                <div class="each_data" data-id="<?php echo $all_row['comment_id']; ?>" data-time="<?php echo $all_row['comment_date_time']; ?>">
                    <div class="content">
                        <div class="each_content">
                            <p><?php echo $all_row['comment_message'] ?></p>
                        </div>
                        <div class="each_content">
                            <?php
                                $all_unformat_date = $all_row['comment_date_time'];
                                $all_format_date = date("Y-n-j hi A", strtotime($all_unformat_date));
                            ?>
                            <h3><?php echo $all_format_date ?></h3>
                        </div>
                        <?php
                            if ($all_row['comment_status'] === "pending") {
                        ?>
                        <div class="each_content button">
                            <button class="btn approve-btn" data-type="comment" data-id="<?php echo $all_row['comment_id']; ?>">Approve</button>
                            <div style="height:20px;"></div>
                            <button class="btn reject-btn" data-type="comment" data-id="<?php echo $all_row['comment_id']; ?>" id="reject">Reject</button>
                        </div>
                        <?php
                            } elseif ($all_row['comment_status'] === "approved") {
                        ?>
                        <div class="each_content button">
            ...
            <div class="table_data" data-status="pending" style="display: none;">
                <?php
                    $pending_sql = "SELECT cm.*, s.student_name
                                FROM comment cm
                                JOIN confession c 
                                ON cm.confession_id = c.confession_id
                                JOIN student s 
                                ...
                                JOIN admin a 
                                ON a.school_id = sc.school_id
                                WHERE c.mode = 'sad' AND cm.comment_status = 'pending' AND a.admin_id = ?
                                ORDER BY cm.comment_date_time DESC;";
                ...
                ?>
                <div class="horizontal"></div>
                <div class="each_data" data-id="<?php echo $pending_row['comment_id']; ?>" data-time="<?php echo $pending_row['comment_date_time']; ?>">
                    <div class="content">
                        <div class="each_content">
                            <p><?php echo $pending_row['comment_message'] ?></p>
                        </div>
                        <div class="each_content">
                            <?php
                                $pending_unformat_date = $pending_row['comment_date_time'];
                                $pending_format_date = date("Y-n-j hi A", strtotime($pending_unformat_date));
                            ?>
                            <h3><?php echo $pending_format_date ?></h3>
                        </div>
                        <?php
                            if ($pending_row['comment_status'] === "pending") {
                        ?>
                        <div class="each_content button">
                            <button class="btn approve-btn" data-type="comment" data-id="<?php echo $pending_row['comment_id']; ?>">Approve</button>
                            <div style="height:20px;"></div>
                            <button class="btn reject-btn" data-type="comment" data-id="<?php echo $pending_row['comment_id']; ?>" id="reject">Reject</button>
                        </div>
                        <?php
                            } elseif ($pending_row['comment_status'] === "approved") {
                        ?>
                        <div class="each_content button">
                            <button class="btn done">Approved</button>
                        </div>
            ...
            <div class="table_data" data-status="approved" style="display: none;">
                <?php
                    $approved_sql = "SELECT cm.*, s.student_name
                                FROM comment cm
                                JOIN confession c 
                                ON cm.confession_id = c.confession_id
                                JOIN student s 
                                ...
                                JOIN admin a 
                                ON a.school_id = sc.school_id
                                WHERE c.mode = 'sad' AND cm.comment_status = 'approved' AND a.admin_id = ?
                                ORDER BY cm.comment_date_time DESC;";
                ...
                ?>
                <div class="horizontal"></div>
                <div class="each_data" data-id="<?php echo $approved_row['comment_id']; ?>" data-time="<?php echo $approved_row['comment_date_time']; ?>">
                    <div class="content">
                        <div class="each_content">
                            <p><?php echo $approved_row['comment_message'] ?></p>
                        </div>
                        <div class="each_content">
                            <?php
                                $approved_unformat_date = $approved_row['comment_date_time'];
                                $approved_format_date = date("Y-n-j hi A", strtotime($approved_unformat_date));
                            ?>
                            <h3><?php echo $approved_format_date ?></h3>
                        </div>
                        <?php
                            if ($approved_row['comment_status'] === "pending") {
                        ?>
                        <div class="each_content button">
                            <button class="btn approve-btn" data-type="comment" data-id="<?php echo $approved_row['comment_id']; ?>">Approve</button>
                            <div style="height:20px;"></div>
                            <button class="btn reject-btn" data-type="comment" data-id="<?php echo $approved_row['comment_id']; ?>" id="reject">Reject</button>
                        </div>
                        <?php
                            } elseif ($approved_row['comment_status'] === "approved") {
                        ?>
                        <div class="each_content button">
                            <button class="btn done">Approved</button>
                        </div>
            ...
            <div class="table_data" data-status="rejected" style="display: none;">
                <?php
                    $rejected_sql = "SELECT cm.*, s.student_name
                                FROM comment cm
                                JOIN confession c 
                                ON cm.confession_id = c.confession_id
                                JOIN student s 
                                ...
                                JOIN admin a 
                                ON a.school_id = sc.school_id
                                WHERE c.mode = 'sad' AND cm.comment_status = 'rejected' AND a.admin_id = ?
                                ORDER BY cm.comment_date_time DESC;";
                ...
                ?>
                <div class="horizontal"></div>
                <div class="each_data"  data-id="<?php echo $rejected_row['comment_id']; ?>" data-time="<?php echo $rejected_row['comment_date_time']; ?>">
                    <div class="content">
                        <div class="each_content">
                            <p><?php echo $rejected_row['comment_message'] ?></p>
                        </div>
                        <div class="each_content">
                            <?php
                                $rejected_unformat_date = $rejected_row['comment_date_time'];
                                $rejected_format_date = date("Y-n-j hi A", strtotime($rejected_unformat_date));
                            ?>
                            <h3><?php echo $rejected_format_date ?></h3>
                        </div>
                        <?php
                            if ($rejected_row['comment_status'] === "pending") {
                        ?>
                        <div class="each_content button">
                            <button class="btn approve-btn" data-type="comment" data-id="<?php echo $rejected_row['comment_id']; ?>">Approve</button>
                            <div style="height:20px;"></div>
                            <button class="btn reject-btn" data-type="comment" data-id="<?php echo $rejected_row['comment_id']; ?>" id="reject">Reject</button>
                        </div>
                        <?php
                            } elseif ($rejected_row['comment_status'] === "approved") {
                        ?>
                        <div class="each_content button">
                            <button class="btn done">Approved</button>
                        </div>
